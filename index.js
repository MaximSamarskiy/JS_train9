...
function findIndexByProperty(arr, name) {
  ...
  const i = arr.findIndex(({ name }) => name >= "Petro");
  return i;
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const filteredSquaredArray = m.filter((m) => m > threshold);
...
function insertElementAt(arr, index, element) {
  ...
  let ins = arr.splice(2, 0, ...element);
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((n) => n);
...
function findDuplicateElements(arr) {
  ...
  const find = arr.filter((x, i) => arr.indexOf(x) !== i);
  return find;
...
function capitalizeWords(arr) {
  ...
  const cap = arr.map((item) => item);

  // Перетворюємо першу літеру слова у верхній регістр да додамо до неї всі символи слова крім першого
  const capi = cap[0].toUpperCase(arr) + cap.slice(1);
  //Повертаємо слово
  const capitalizedArray = ["Apple", "Banana", "Orange"];
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((total, person) => total + person.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex((arr) => arr < 7);
